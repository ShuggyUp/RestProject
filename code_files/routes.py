...
def print_vacancies() -> dict:
    """Выводит все данные о вакансиях со значением указанным в limit_record"""
    engine = create_engine(f'postgresql://{db_settings.db_user}:{db_settings.db_pass}@{db_settings.db_host}:'
                           f'{db_settings.db_port}/{db_settings.db_name}')
    vacancies_db_manager = VacanciesDatabaseManager(engine, Vacancies)
    vacancies_handler_api = VacanciesHandlerAPI(vacancies_db_manager)
...
def search_vacancies(search_text: str = 'Разработчик') -> dict:
    ...
    keywords_like_query = '%' + '%'.join(search_text.lower().split()) + '%'
